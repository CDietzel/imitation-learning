...
algorithms = ["PPO", "BC", "IBC", "GAIL", "AIRL", "FAIRL", "GMMIL", "RED", "DRIL"]
# algorithms = ['PPO']
...
BASELINE["ant"] = [570.80, 104.82]
BASELINE["halfcheetah"] = [787.35, 104.31]
BASELINE["hopper"] = [1078.36, 325.52]
BASELINE["walker2d"] = [1106.68, 417.79]
ENV_NAMES = dict()
ENV_NAMES["ant"] = "Ant"
ENV_NAMES["halfcheetah"] = "HalfCheetah"
ENV_NAMES["hopper"] = "Hopper"
ENV_NAMES["walker2d"] = "Walker2D"
    ...
    for env in envs:
        ...
        for alg in algorithms:
            try:
                ...
                print(
                    "Error: Could not load data from environment: "
                    + env
                    + " and algorithm: "
                    + alg
                )
...
def create_all_plots(x, y, save_fig=False):
    ...
    fig.legend(
        handles, labels, loc="lower center", ncol=len(algorithms) + 1, fontsize=fontsize
    )
...
def create_hyperparam_plot(x, y, save_fig=False):
    ...
    # algorithms = ['BC', 'IBC', 'GMMIL', 'RED', 'DRIL', 'AIRL', 'FAIRL', 'GAIL'][::-1]
