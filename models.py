...
def _create_fcnn(
    ...
    for l in range(len(network_dims) - 1):
        ...
        nn.init.orthogonal_(
            layer.weight, gain=nn.init.calculate_gain(activation_function)
        )
...
class GMMILDiscriminator(nn.Module):
    ...
    def predict_reward(self, state, action, expert_state, expert_action):
        ...
        expert_state_action = (
            expert_state
            if self.state_only
            else _join_state_action(expert_state, expert_action, self.action_size)
        )

        # Use median heuristics to set data-dependent bandwidths
        if self.gamma_1 is None:
            self.gamma_1 = (
                1 / _squared_distance(state_action, expert_state_action).median().item()
            )
            self.gamma_2 = (
                1
                / _squared_distance(
                    expert_state_action.transpose(0, 1),
                    expert_state_action.transpose(0, 1),
                )
                .median()
                .item()
        ...
        similarity = _gaussian_kernel(
            expert_state_action, state_action, gamma=self.gamma_1
        ).mean(dim=0) + _gaussian_kernel(
            expert_state_action, state_action, gamma=self.gamma_2
        ).mean(
            dim=0
        )
        return (
            similarity
            - (
                _gaussian_kernel(state_action, state_action, gamma=self.gamma_1).mean(
                    dim=0
                )
                + _gaussian_kernel(state_action, state_action, gamma=self.gamma_2).mean(
                    dim=0
                )
            )
...
class AIRLDiscriminator(nn.Module):
    def __init__(
        ...
        self.g = nn.Linear(
            state_size if state_only else state_size + action_size, 1
        )  # Reward function r
    ...
    def reward(self, state, action):
        if self.state_only:
            ...
            return self.g(_join_state_action(state, action, self.action_size)).squeeze(
                dim=1
            )
    ...
    def forward(self, state, action, next_state, log_policy, terminal):
        ...
        return (
            f - log_policy
        )  # Note that this is equivalent to sigmoid^-1(e^f / (e^f + π))
    ...
    def predict_reward(self, state, action, next_state, log_policy, terminal):
        ...
        return torch.log(D + 1e-6) - torch.log1p(
            -D + 1e-6
        )  # Add epsilon to improve numerical stability given limited floating point precision
...
class REDDiscriminator(nn.Module):
    ...
    def set_sigma(self, expert_state, expert_action):
        ...
        self.sigma_1 = (
            1
            / _squared_distance(prediction.transpose(0, 1), target.transpose(0, 1))
            .median()
            .item()
        )
