...
from training import (
    ...
    behavioural_cloning_update,
    implicit_behavioural_cloning_update,
    ppo_update,
...
def main(cfg: DictConfig) -> None:
    ...
    agent = ActorCritic(
        ...
        log_std_dev_init=cfg.model.log_std_dev_init,
    )
    agent_optimiser = optim.RMSprop(
        agent.parameters(), lr=cfg.reinforcement.learning_rate, alpha=0.9
    )
    ...
    if cfg.algorithm in ["AIRL", "DRIL", "FAIRL", "GAIL", "GMMIL", "PUGAIL", "RED"]:
        if cfg.algorithm == "AIRL":
            discriminator = AIRLDiscriminator(
                ...
                action_size,
                cfg.model.hidden_size,
                cfg.reinforcement.discount,
                state_only=cfg.imitation.state_only,
            )
        elif cfg.algorithm == "DRIL":
            discriminator = Actor(
                state_size, action_size, cfg.model.hidden_size, dropout=0.1
            )
            ...
            discriminator = GAILDiscriminator(
                ...
                cfg.model.hidden_size,
                state_only=cfg.imitation.state_only,
                forward_kl=cfg.algorithm == "FAIRL",
            )
        elif cfg.algorithm == "GMMIL":
            discriminator = GMMILDiscriminator(
                state_size,
                action_size,
                self_similarity=cfg.imitation.self_similarity,
                state_only=cfg.imitation.state_only,
            )
        ...
        if cfg.algorithm in ["AIRL", "DRIL", "FAIRL", "GAIL", "PUGAIL", "RED"]:
            discriminator_optimiser = optim.RMSprop(
                discriminator.parameters(), lr=cfg.imitation.learning_rate
            )

    # Metrics
    metrics = dict(train_steps=[], train_returns=[], test_steps=[], test_returns=[])
    recent_returns = deque(
        maxlen=cfg.evaluation.average_window
    )  # Stores most recent evaluation returns
    ...
    for step in pbar:
        ...
        if cfg.algorithm in ["BC", "IBC", "DRIL", "RED"]:
            if step == 1:
                for _ in tqdm(range(cfg.imitation.epochs), leave=False):
                    if cfg.algorithm == "BC":
                    ...
                    elif cfg.algorithm == "IBC":
                        # Perform implicit behavioural cloning updates offline
                        implicit_behavioural_cloning_update(
                            agent,
                            expert_trajectories,
                            agent_optimiser,
                        ...
                        with torch.no_grad():  # TODO: Check why inference mode fails?
                            discriminator.set_uncertainty_threshold(
        ...
        if not cfg.algorithm in {"BC", "IBC"}:
            ...
            if len(trajectories) >= cfg.training.batch_size:
                policy_trajectories = flatten_list_dicts(
                    trajectories
                    ...
                    if cfg.algorithm in ["AIRL", "FAIRL", "GAIL", "PUGAIL"]:
                        ...
                        for _ in tqdm(range(cfg.imitation.epochs), leave=False):
                            adversarial_imitation_update(
                                ...
                                cfg.get("pos_class_prior", 0.5),
                                cfg.get("nonnegative_margin", 0),
                            )

                    # Predict rewards
                    states, actions, next_states, terminals = (
                        policy_trajectories["states"],
                        policy_trajectories["actions"],
                        torch.cat([policy_trajectories["states"][1:], next_state]),
                        policy_trajectories["terminals"],
                    )
                    with torch.inference_mode():
                        if cfg.algorithm == "AIRL":
                            policy_trajectories[
                            ...
                            policy_trajectories[
                                "rewards"
                            ] = discriminator.predict_reward(states, actions)
                        elif cfg.algorithm in ["FAIRL", "GAIL", "PUGAIL"]:
                            policy_trajectories[
                                "rewards"
                            ] = discriminator.predict_reward(states, actions)
                ...
                for _ in tqdm(range(cfg.reinforcement.ppo_epochs), leave=False):
                    ppo_update(
                        ...
                        agent_optimiser,
                        cfg.reinforcement.discount,
                        cfg.reinforcement.trace_decay,
                        cfg.reinforcement.ppo_clip,
                        cfg.reinforcement.value_loss_coeff,
                        cfg.reinforcement.entropy_loss_coeff,
        ...
        if step % cfg.evaluation.interval == 0 and not cfg.check_time_usage:
            ...
            if cfg.algorithm in {"BC", "IBC"}:
                lineplot(
                    range(
                        cfg.evaluation.interval, cfg.steps + 1, cfg.evaluation.interval
                    ),
                    metrics["test_returns"] * (cfg.steps // cfg.evaluation.interval),
                    "test_returns",
                )
                break
            elif len(metrics["train_returns"]) > 0:  # Plot train returns if any
                lineplot(
                    metrics["train_steps"], metrics["train_returns"], "train_returns"
                )
        elif cfg.algorithm in {"BC", "IBC"} and cfg.check_time_usage:
            break
    ...
    env.close()
    return sum(recent_returns) / float(
        1 if cfg.algorithm in {"BC", "IBC"} else cfg.evaluation.average_window
    )
