...
class PendulumEnv:
    def __init__(self, env_name=""):
        ...
        )  # Convert action space for action clipping
    ...
    def step(self, action):
        action = action.clamp(
            min=self.env.action_space.low, max=self.env.action_space.high
        )  # Clip actions
        state, reward, terminal, _ = self.env.step(
            action[0].detach().numpy()
        )  # Remove batch dimension from action
        return (
            torch.tensor(state, dtype=torch.float32).unsqueeze(dim=0),
            reward,
            terminal,
        )  # Add batch dimension to state
    ...
    def get_dataset(self, size=0, subsample=20):
        ...
        dataset_out = {
            "states": torch.as_tensor(
                dataset["observations"][:-1], dtype=torch.float32
            ),
            ...
            "terminals": torch.as_tensor(
                dataset["terminals"][:-1], dtype=torch.float32
            ),
        }
        # Postprocess
